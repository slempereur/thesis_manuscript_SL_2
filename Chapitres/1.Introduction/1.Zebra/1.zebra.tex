...
\sectionmark{Le \pz{}, second modèle le plus utilisé}
...
%
Elle permet par exemple d'utiliser moins d'anticorps pour effectuer le marquage \ihc{} d'échantillons entiers ou permet de réduire le nombre de champs optiques nécessaires pour la réalisation d'imagerie d'individus entiers.
}
%%
%
...
%
\ajout{
L'alevin vésiculé de \pz{} à 5dpf, aussi appelé eleuthéroembryon (EE), est ainsi largement employé en toxicologie.
}
%
...
\ajout{
La \autoref{table:zebra} résume les caractéristiques du \pz{} ainsi que les avantages que l'on peut en tirer.
...
\scriptsize
    \begin{tabularx}{\textwidth}{YY}
        \toprule
        \tableheadline{Caractéristiques}
        &
        \tableheadline{Avantages}
        \\
        \toprule
        Petite taille
        &
        Faible coût d'entretien et d'imagerie
        \\
        \midrule
        Haute capacité de reproduction
        &
        Grande taille d'échantillons \newline permettant une haute puissance statistiques \newline capable de détecter \newline des phénomènes biologiques discrets
        \\
        \midrule
        ...
        Modèle bien établi
        &
        Vaste quantité de ressources \newline et de bases de données
        \\
        \midrule
        Possibilité de création de mutants \newline grâce à une large variété d'outils de modification du génome
        &
        Possibilité d'étude d'un gène précis
        \\
        \bottomrule
    \end{tabularx}
    \caption{
    La \pz{} présente de nombreux avantages expliquant sa large utilisation dans la communauté scientifique
    }
    \label{table:zebra}
\end{table}
}

\sectionmark{Présentations d'autres modèles animaux}
